...
model.load_state_dict(
    torch.load("app/model_number_2.pt", map_location=torch.device("cpu"))
)
model.eval()
...
def predict():
    try:
        ...
        # Subtract 1 from predicted index
        return jsonify({"prediction": Classification_label[int(predicted) - 1]})
